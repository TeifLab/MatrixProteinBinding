...
for ThisSiteNumber = 1:Lpolymer
    ...
    if (StatSum>threshold || Trace1> threshold)
        LeftVectdK = LeftVectdK/threshold;
        LeftVectdE = LeftVectdE/threshold;
        LeftVect= LeftVect/threshold;
        StatSum = StatSum/threshold;
        NormCount = NormCount+1;
    end
    
    %StatSum calculation:
    
    [Q, dQdK, dQdE] = MatrixInitProteinBinding(ThisSiteNumber,TestSiteNumber); %%CHECK THIS
    ...
    for g=1:fNumberOfLigands
        ...
        % switch to permute as faster -v3
        LVDKg1z = permute(LeftVectdK(g,1,:),[3 1 2])';
        dQdKgzz = permute(dQdK(g,:,:),[2 3 1]);
        
        %if max(max((LVDKg1z-LVDKg1z1).^2))
        %    error('Different permute shiftdim LVDKg1z');
        %end
        %if max(max((dQdKgzz-dQdKgzz1).^2))
        %    error('Different permute shiftdim dQdKgzz');
        %end
    ...
    for e=1:eNumberOfChromatinStates
        ...
        LVDEe1z = permute(LeftVectdE(e,1,:),[3 1 2])';
        dQdEezz = permute(dQdE(e,:,:),[2 3 1]);
        
        %if max(max((LVDEe1z-LVDEe1z1).^2))
        %    error('Different permute shiftdim LVDEe1z');
        %end
        %if max(max((dQdEezz-dQdEezz1).^2))
        %    error('Different permute shiftdim dQdEezz');
        %end
...
c(1)=1.-1./StatSum*(threshold^NormCount);
